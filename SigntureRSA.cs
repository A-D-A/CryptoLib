...
namespace CryproLib
{
    public class SigntureRSA : ISign
    {
        IHash hashFunction;
        static BigInteger N;
        ...
        private bool Check(Message msg, BigInteger d, BigInteger N)
        {
            byte[] buf = Encoding.Default.GetBytes(msg.message);
            BigInteger[] w = new BigInteger[msg.signature.Length];
            for (int i = 0; i < msg.signature.Length; ++i)
            {
                w[i] = Power(msg.signature[i], d, N);
            }
